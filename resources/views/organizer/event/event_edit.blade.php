...
<div class="col-12">
    <div class="card">
        <div class="card-body">
            ...
            <form method="post" action="{{ route('update.event1', $events->id) }}" enctype="multipart/form-data">
                @csrf
        ...
        <select name="event_category_id" class="form-select" aria-label="Default select example">
            ...
            <option value="{{ $cat->id }}" {{ $cat->id == $events->event_category_id ? 'selected' : '' }} >{{ $cat->event_category }}</option>
            @endforeach
            </select>
               </div>
            </div>
            <!-- end row -->
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input name="event_title" value="{{ $events->event_title }}" class="form-control" type="text" id="example-text-input"> 
                </div>
            </div>
            <!-- end row -->
           ...
           <input name="event_image" class="form-control" type="file" id="image">
            ...
            <!-- end row -->
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
  <img id="showImage" class="rounded avatar-lg" src="{{ asset($events->event_image) }}" alt="Card image cap">
                </div>
            </div>
            <!-- end row -->
<input type="submit" class="btn btn-info waves-effect waves-light" value="Update Event Data">
            </form>
